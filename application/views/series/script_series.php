<script>
    ...
    function handle_series()
    {
        $.ajax({
            ...
            beforeSend: function()
            {
                $("#load_series").html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span>');
            },
            success: function (data) 
            {
                ...
                $("#load_series").html('');
                $('#judul_series').append(title);
                $('#series').append(`<div class="embed-responsive embed-responsive-16by9">
                                        <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/`+url.substring(32) +`" allowfullscreen></iframe>
                                    </div>`);
                $('#judul_series_detail').append(`Nobar App Nonton `+ title +` Film Subtitle Indonesia Streaming Movie Download Gratis Online`);
                $('#poster').append(`<img src="`+ img +`" class="w-100">`);
                $('#info_series').append(`<table>
                                            <tr>
